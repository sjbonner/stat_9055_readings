...
\fancyhf{}
\lfoot{Copyright Simon Bonner, University of Western Ontario, 2023}
\rfoot{Page \thepage}
...
\begin{center}
  \begin{Large}
    ...
    \medskip
    {\bf Section 4: Introduction Generalized Linear Models}\\
    {\bf Version 2023.1}
\end{Large}
\end{center}
...
\item apply the iterativley (re-)weighted least squares algorithm to compute the MLEs, and
...
\section{Generalized Linear Model Framework}
...
\section{Maximum Likelihood Inference for GLMs}
\label{sec:mles-for-glm}
...
\end{enumerate}
Substituting these elements back into equation \eqref{eq:le1} yields
\[
...
\subsection{Asymptotic Covariance Matrix}
\label{sec:cov-for-glm}
...
\]
The matrix in equation \eqref{eq:obsinf} is found by replacing the random variable $\bm Y$ with the observed value $\bm y$ and removing the expected value (since there are no longer any random quantities). For this reason, the matrix is called the observed information matrix.
...
\subsection{ML as Reweighted Least Squares}
...
% \subsection{Simplifications for Canonical Link Functions}
