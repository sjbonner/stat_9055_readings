...
\begin{center}
  \begin{Large}
    ...
    \medskip
    {\bf Section 3: Maximum Likelihood Inference Part 2}\\
...
\section{Likelihood Based Hypothesis Tests}
...
\section{Likelihood Based Confidence Intervals}
...
\]
The plot in Figure \ref{fig:example1} show the value of the likelihood ratio test statistic as a function of $\mu$. The horizontal line represents the value of $\chi^2_{1,.05}=3.841$. Inverting the test statistic to construct the confidence interval is equivalent to finding all of the value of $\mu$ for which the test statistic is below this point. This is done by finding the two points where the lines cross, which are $.658$ and $1.445$. Hence, the 95\% Wald confidence interval for $\mu$ is (.578,1.322) and the 95\% likelihood ratio confidence interval is (.658,1.445). 

\begin{figure}
  \centering
  \includegraphics{example_figure}
  \caption{Likelihood ratio test statistic for a random sample of size 25 from an exponential distribution when $\bar x=.95$. The vertical dashed line represents the MLE (i.e., $\bar x$). The horizontal grey line represents the value $\chi^2_{1,.05}=3.841$.}
  \label{fig:example1}
\end{figure}
...
% Suppose that $Y_1,\ldots,Y_n$ are independent Bernoulli random variables with probability of success $p$. 
% \begin{enumerate}[a)]
% \item Construct the likelihood equation.
% \item Derive the maximum likelihood estimator, $\hat p$.
% \item Obtain the asymptotic distribution of $\hat p$.
% \item Determine the standard error of $\hat p$.
% \item Construct the Wald test statistic for the hypotheses
%   \[
%   H_0:p=p_0 \mbox{ vs } H_a:p \neq p_0
%   \]
%   and describe its asymptotic distribution.
% \item Invert the Wald test statistic to obtain an approximate confidence interval for $p$. 
% \end{enumerate}

% \item Multinomial Distribution

% The multinomial distribution generalizes the binomial distribution to more than two categories. Suppose that we observe $n$ data points each falling into one of $c$ categories independently with probabilities $\bm \pi=(\pi_1,\ldots,\pi_c)$ (note that $\sum_{j=1}^c \pi_c=1$). Let $\bm X=(X_1,\ldots,X_c)'$ record the number of observations in each category. The density function is:
% \[
% f(\bm x|\bm \pi)=\frac{n!}{\prod_{j=1}^c x_j!} \prod_{j=1}^c \pi_j^{x_c}. 
% \]
% \begin{enumerate}[a)]
% \item Construct the likelihood function.
% \item Derive the maximum likelihood estimator, $\hat {\bm \pi}$. (Hint: think carefully about the number of parameters in this model.)
% \item Obtain the asymptotic distribution of $\hat {\bm \pi}$.
% \item Construct the likelihood ratio statistic for testing
%   \[
%   H_0:\bm \pi=\bm \pi_0 \mbox{ vs } H_a:\bm \pi \neq \bm \pi_0
%   \]
%   and find its asymptotic distribution.

% \item In genetics, a gene represents a location on a chromosome and the different forms the gene can take are called alleles. A genotype is defined by the pair of alleles an individual has -- one allele from their mother and one from their father. A gene with two alleles, labelled $A$ and $a$, has four different genotypes: $AA$, $Aa$, and $aa$. Let $p$ and $q=1-p$ denote the proportion of all alleles in a population that are $A$ and $a$ respectively. Let $f(X)$ denote the proportion of individuals in the population with genotype $X$. The gene is in Hardy-Weinberg equilibrium if:
% \[
% f(AA)=p^2, \quad f(Aa)=2pq, \quad f(aa)=q^2.
% \]
% In this case, the genotype frequencies in a large population will remain constant from one generation to the next unless acted upon by outside selection.

% The amount of spotting in Scarlet tiger moths is largely determined by a gene with two alleles. Individuals of genotype $AA$ are white-spotted, individuals with genotype $aa$ have little spotting, and individuals of genotype $Aa$ are intermediate. A study of 1612 moths conducted in 1971 observed the following counts:
% \begin{center}
%   \begin{tabular}{l|ccc|c}
%     & White-spotted ($AA$) & Intermediate ($Aa$) & Little spotting ($aa$) & Total\\
%     \hline
%     Number & 1469 & 138 & 5 & 1612
%   \end{tabular}
% \end{center}

% Is the system in Hardy-Weinberg equilibrium?
% \end{enumerate}
% \end{enumerate}
